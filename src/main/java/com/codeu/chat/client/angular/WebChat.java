...
// WebChat - top-level client application.
@Path("webchat")
public final class WebChat {

  private final static Logger.Log LOG = Logger.newLog(WebChat.class);
  ...
  // Constructor - sets up the WebChat Application
  public WebChat(Controller controller, View view) {
  ...
  public String getIt() {
      return "This text is being sent from java backend, but from the WebClient class!";
  }
  ...
  // Old code (meaning code from codeu.commandline.Chat) from this point on:
  ...
  public void selectConversation(Scanner lineScanner) {
    ...
    if (selectionSize == 0) {
      ...
      final WebListNavigator<ConversationSummary> navigator =
          new WebListNavigator<ConversationSummary>(
